...
/* using 'and' (dayOfWeek > 0 && dayOfWeek < 6)*/
/* using 'or' */
...
// Step 6: Using an else statement, set the message variable to 'Woohoo!  It is the weekend!'
  ...
  text = "Woohoo!  It is the weekend!"
};


/* SWITCH, CASE, BREAK message2 */
...
document.getElementById("message2").innerHTML = statement;
...
function sortBy(){
  ...
  let name = templeList.templeName;
  element.innerHTML = "";
  
  let sorted = templeList.sort((temple1, temple2) => {
    if (order === "templeNameAscending")
      if (temple1[name] > temple2[name])
        return 1
      else if (temple1[name] < temple2[name])
        return -1
      else
        return 0;
  ...
  let sortedList = sorted.forEach(temple => {element.innerHTML += temple});
  output(sortedList);
